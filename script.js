        ...
        .then(res => {
            ...
            res.data.Pizzas.forEach(element => {
                ...
                fila.innerHTML = `<th>${element.Id}</th>
                            ...
                            <td><button onclick="verMas(${element.Id})" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modalPizza">Ver Mas</button></td>`;
...
input.onkeyup = (e) => {
    ...
    if (e.target.value) {
        ...
        while (i < lista_pizzas.length) {
            if (e.target.value.toLowerCase() == lista_pizzas[i].Nombre.substring(0, e.target.value.length).toLowerCase()) {
